...
- **Wire-shape contract gate** (`.github/workflows/wire-shape-contract.yml`). CI fails any PR that introduces drift between the plugin's wire-bound TypeScript interfaces (`MCPCheckInputResponse`, `MCPCheckOutputResponse`, `ExplainPolicy`, `ExplainRule`, `DecisionExplanation`, `CreateOverrideResult`) and the AxonFlow agent's OpenAPI spec at the SHA pinned in `tests/fixtures/wire-shape-baseline.json`. Drift NOT covered by the baseline blocks merge. The pinned SHA is itself guarded by a `spec-pin-bump` PR label so a single PR can't both move the SHA and silence drift. Mirrors the four SDK gates (TS / Python / Go / Java) per ADR-047.
- **Install-to-use smoke gate** (`.github/workflows/install-smoke.yml`). CI now packs the plugin tarball on every PR, installs it into a clean Node project, validates the file list (`package.json`, `openclaw.plugin.json`, `dist/`, `policies/`, README/CHANGELOG/LICENSE), confirms `AxonFlowClient` is exported, and exercises `mcpCheckInput` against a local stub agent for both deny (SQLi) and allow (benign) paths. Catches the class of regressions unit tests miss because they run against the source tree (incomplete `package.json::files`, broken top-level export, manifest structural drift, Plugin Batch 1 field regressions). No external network or live AxonFlow stack required. Behavioural counterpart to the wire-shape gate above — install-smoke catches publish-path drift; wire-shape catches type-vs-spec drift.
- **`CONTRIBUTING.md`** with the baseline burndown-or-justify policy and the wire-shape Cat C three-bucket framework. Mirrors the conventions adopted for the four AxonFlow SDKs (see ADR-047).
