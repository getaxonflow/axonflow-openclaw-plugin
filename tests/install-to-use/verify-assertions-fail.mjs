...
// The shape an AxonFlow v11.0.0 check-input deny has (measured): no
// risk_level, and override_available never true.
const goodDeny = {
  allowed: false,
  decision_id: 'dec-deadbeef',
  block_reason: 'explicit_constraint',
  policy_matches: [{ policy_id: 'corpus:static_policies:sys__dangerous__destructive__fs', policy_name: 'Destructive Filesystem Operations' }],
};
...
expectNoThrow('deny: well-formed response passes', () => assertPolicyDeny(goodDeny));
...
expectThrows('deny: rejects allowed=true (would let a denied command through)', () => {
  assertPolicyDeny({ ...goodDeny, allowed: true });
}, 'allowed=false');
...
expectThrows('deny: rejects missing decision_id', () => {
  assertPolicyDeny(omit(goodDeny, 'decision_id'));
}, 'decision_id');

expectThrows('deny: rejects empty decision_id', () => {
  assertPolicyDeny({ ...goodDeny, decision_id: '' });
}, 'decision_id');

expectThrows('deny: rejects missing block_reason', () => {
  assertPolicyDeny(omit(goodDeny, 'block_reason'));
}, 'block_reason');

expectThrows('deny: rejects empty block_reason', () => {
  assertPolicyDeny({ ...goodDeny, block_reason: '' });
}, 'block_reason');

expectThrows('deny: rejects empty policy_matches array', () => {
  assertPolicyDeny({ ...goodDeny, policy_matches: [] });
}, 'policy_matches');

expectThrows('deny: rejects missing policy_matches', () => {
  assertPolicyDeny(omit(goodDeny, 'policy_matches'));
}, 'policy_matches');

expectThrows('deny: rejects null response', () => assertPolicyDeny(null), 'not an object');
...
expectThrows('deny: rejects non-string decision_id (number)', () => {
  assertPolicyDeny({ ...goodDeny, decision_id: 42 });
}, 'decision_id');

expectThrows('deny: rejects non-string block_reason (number)', () => {
  assertPolicyDeny({ ...goodDeny, block_reason: 5 });
}, 'block_reason');

expectThrows('deny: rejects non-array policy_matches (object)', () => {
  assertPolicyDeny({ ...goodDeny, policy_matches: { count: 1 } });
}, 'policy_matches');

expectThrows('deny: rejects non-array policy_matches (string)', () => {
  assertPolicyDeny({ ...goodDeny, policy_matches: 'sql-injection' });
}, 'policy_matches');
...
expectThrows('deny: rejects empty-string response', () => assertPolicyDeny(''), 'not an object');
