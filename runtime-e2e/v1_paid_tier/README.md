...
2. **`recover` CLI (W3 free recovery).** Driving `bin/axonflow-openclaw-recover` posts to `/api/v1/recover`, extracts the magic-link token from the agent's capture file, calls `/api/v1/recover/verify`, persists `try-registration.json` at `$AXONFLOW_CONFIG_DIR` with mode 0o600, and the new credentials authenticate against `$AXONFLOW_ENDPOINT/api/request` (the agent's primary Basic-auth surface — works against any community-saas deployment without depending on the orchestrator's optional `AXONFLOW_INTERNAL_SERVICE_SECRET` setting).
...
Optional — pin a specific synthetic source-IP (sent via `X-Forwarded-For`) for the agent's per-IP rate limiter on `/api/v1/register` and `/api/v1/recover`. Default is a random `10.99.x.y` per run, which is the right answer in almost all cases. Set this only when an upstream proxy strips XFF or when the random IP collides with another concurrent test:

```bash
RUNTIME_E2E_XFF=10.99.42.7 \
  bash runtime-e2e/v1_paid_tier/test.sh
```
