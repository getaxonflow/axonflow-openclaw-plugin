...
**AxonFlow is self-hosted.** It runs on your infrastructure via Docker Compose. All policy evaluation, PII detection, and audit logging happens on your own AxonFlow instance. Credentials are only needed for enterprise mode — community mode requires no auth. Telemetry is automatically suppressed for localhost/loopback deployments. For non-local endpoints, an anonymous startup ping (version and basic deployment info) is sent by default — opt out globally with `DO_NOT_TRACK=1`.
