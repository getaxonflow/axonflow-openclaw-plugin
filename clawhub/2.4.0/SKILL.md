...
> **Plugin v2.4.0+:** the operations below are also exposed to the OpenClaw agent as registered tools. Total of 11 agent-callable tools, split into two groups:
>
> - **Read-side governance** (6): `axonflow_explain_decision`, `axonflow_list_recent_decisions` (NEW in v2.4.0 — paginated list of recent governance decisions tier-throttled per the platform's Free 24h/5 vs Pro 30d/100 retention windows), `axonflow_list_overrides`, `axonflow_create_override`, `axonflow_revoke_override`, `axonflow_audit_search`. The agent can answer "what just got blocked?" / "show me my recent decisions" / "why was this blocked?", create / list / revoke session overrides, and search audit events.
...
>
> **Decision History API (platform v7.9.0+):** the new `axonflow_list_recent_decisions` tool wraps the platform's `GET /api/v1/decisions` list endpoint. Every decision driven by a static-policy match also records `policy_version_at_decision` inline on the audit row, so explain calls surface the policy text that actually fired even after the policy is later edited. Free 24h / 5 results / page; Pro 30d / 100 results / page. Free callers hitting the cap see the upgrade envelope rendered to the OpenClaw host.
